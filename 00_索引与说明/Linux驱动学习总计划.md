...
> 当前起点：第 8 课显示链路只读盘点与问答验收已完成；第 9 课尚未开始
...
- VOP、HDMI、MIPI DSI、panel 的角色梳理；
- framebuffer / DRM、`/dev`、`/sys` 和 `dmesg` 的区别；
- 运行时设备树、驱动绑定、连接器状态和 endpoint 图的只读观察；
- 逐题概念复盘与验收。

实际结论：VOP 与 HDMI 已绑定，HDMI 未连接显示器；两个 DSI 节点均为 `disabled`，未确认到活动的 panel/backlight 链路。全程未改 DTS、未烧录、未操作未知 GPIO。endpoint 图留待设备树进阶课程深入。

#### 第 9 课：官方 3.1 寸 MIPI 镜像基线（尚未开始）
